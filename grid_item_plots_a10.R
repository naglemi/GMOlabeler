...
wd <- paste0("/scratch2/NSF_GWAS/GMOlabeler/plots/", opt$datapath1)
if(!dir.exists(wd)) dir.create(wd, recursive = TRUE)
setwd(wd)

#datapath <- paste0("/scratch2/NSF_GWAS/GMOlabeler/output/", opt$datapath1, "stats.csv")
datapath <- paste0("/scratch2/NSF_GWAS/GMOlabeler/output/", opt$datapath1, "stats_with_sums_over_tissues.csv")
...
if(opt$missing==TRUE){
  ...
  output <- output[which(output$present!="N"),]
  output <- output[which(output$present!="C"),]
  output <- output[which(output$present!="P"),]
  cat("\n")
...
ggsave(
  ...
  plot = last_plot(),   width = 4,   height = opt$height,   units = "in")
...
fwrite(randomization_datasheet,
       paste0("/scratch2/NSF_GWAS/GMOlabeler/output/", opt$datapath1, "plants_over_plates.csv")
)
