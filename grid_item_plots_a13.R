...
option_list = list(
  ...
  make_option(c("-w", "--width"),
              ...
              metavar="numeric"),
  make_option(c("-z", "--gmolabeler_path"),
              type="string",
              default=NULL,
              help="Path to GMOlabeler Python library",
              metavar="string")
);
...
wd <- paste0(gmolabeler_path, "/plots/", opt$datapath1)
...
datapath <- paste0(gmolabeler_path, "/output/", opt$datapath1, "stats_with_sums_over_tissues.csv")
...
fwrite(randomization_datasheet,
       paste0(gmolabeler_path, "/output/", opt$datapath1, "plants_over_plates.csv")
)
